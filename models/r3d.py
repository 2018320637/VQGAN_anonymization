...
class mlp(nn.Module):
    ...
    def __init__(self, final_embedding_size = 128, use_normalization = True):
        ...
        self.relu = nn.ReLU(inplace=True)
        self.fc2 = nn.Linear(512, self.final_embedding_size)
    ...
    def forward(self, x):
        ...
        if clip_type == 'd':
            ...
            #print('Dealing with Sparse clip')

            # Global Sparse Representation

            gsr = self.temp_avg(x)
            gsr = gsr.flatten(1)
            #print(f'After flattening x shape is {gsr.shape}')
            gsr = self.relu(self.fc1(gsr))
            #print(f'After fc1, shape of x is {gsr.shape}')
            gsr = nn.functional.normalize(self.fc2(gsr), p=2, dim=1)
            #print(f'After fc2, shape of x is {gsr.shape}')
            #print(f'Shape of x is {x.shape}')
            # exit()
            # Local Sparse Representations
            x1, x2, x3, x4 = [nn.functional.normalize(self.fc2(\
                                self.relu(self.fc1(x[:,:,i,:,:].flatten(1))))) for i in range(4)]
            ...
            #print('Clip type is not given properly!!!')
